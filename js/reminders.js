    ...
    function render() {
        ...
        list.innerHTML = _reminders.map(function(r) {
            ...
            return '<div class="bg-white rounded-xl shadow-sm p-4 flex items-center gap-4 border-l-4 ' + (isDone ? 'border-gray-300' : isDue ? 'border-red-500' : 'border-pet-DEFAULT') + ' transition hover:shadow-md">'
                + '<div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ' + (isDone ? 'bg-gray-100 text-gray-400' : isDue ? 'bg-red-100 text-red-500' : 'bg-pet-light text-pet-DEFAULT') + '"><i class="fa-solid ' + (isDone ? 'fa-check' : 'fa-bell') + '"></i></div>'
                ...
                + (isDone ? '' : '<button onclick="Reminders.complete(' + r.task_id + ')" class="px-3 py-1.5 text-xs text-green-600 bg-green-50 rounded-lg hover:bg-green-100 transition"><i class="fa-solid fa-check mr-1"></i>เสร็จแล้ว</button>')
                + '<button onclick="Reminders.remove(' + r.task_id + ')" class="px-2 py-1.5 text-xs text-red-500 hover:bg-red-50 rounded-lg transition"><i class="fa-solid fa-trash"></i></button>'
                + '</div></div>';
        }).join('');
    }
